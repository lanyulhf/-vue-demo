<template>
    <div class="food_container">
    	<headTop :head-title="headTitle" goBack="true" class="headTop"></headTop>
    	<section class="sort_container">
    		...
    		<div class="sort_item"  :class="{choose_type:sortBy == 'food'}">
	    		...
	    		<transition v-show="category" name="showlist">
              ...
              <section class="category_left">
	    					<ul>
	    						<li v-for="(item, index) in category" :key="index" class="category_left_li" @click="selectCategoryName(item.id,index)" :class="{category_active:restaurant_category_id == item.id}">
									<section>
										<img :src="getImgPath(item.image_url)" v-if="index" id="category_icon">
										<span>{{item.name}}</span>
									</section>
									<section>
      ...
      <transition name="showcover">
    	...
    	<section class="shop_list_container">
	    	<shopList :geohash="geohash" :restaurantCategoryId="restaurant_category_id" :restaurantCategoryIds="restaurant_category_ids" :sortByType='sortByType' :deliveryMode="delivery_mode" :confirmSelect="confirmStatus" :supportIds="support_ids" v-if="latitude"></shopList>
    	</section>
    </div>
</template>
...
import { msiteAddress, foodCategory,foodDelivery , foodActivity} from "../service/getData.js";
  ...
  methods:{
   ...mapMutations(['RECORD_ADDRESS']),
    async initData(){
      ...
      //获取筛选列表的配送方式
      let deliveryRes  = await foodDelivery(this.latitude, this.longitude);
      this.Delivery = deliveryRes.data;
      //获取筛选列表的商铺活动
      let activityRes = await foodActivity(this.latitude, this.longitude);
    ...
    getCategoryIds(id,name){
      ...
      this.sortBy = "";
      //  console.log("a"+this.restaurant_category_id)
      //  console.log("b"+this.restaurant_category_ids)
  ...
  .sort_list_container {
    ...
    .sort_list_li {
      ...
      p {
        ...
        text-align: left;
        text-indent: 0.1rem;
        border-bottom: 0.003rem solid #999;
        display: flex;
	      justify-content:space-between;
        align-items: center;
        span {
